...
def _reinit_hf_config_class_inplace(vllm_config) -> None:
    """Re-initialize HF config instance to the current class to ensure picklability.

    In spawned processes, dynamic HF modules (trust_remote_code) can be loaded
    under different module names, causing pickle to fail with
    "it's not the same object" errors. Reconstructing the config using the
    current class' from_dict/to_dict rebinding avoids that.
    """
    model_config = getattr(vllm_config, "model_config", None)
    hf_cfg = getattr(model_config, "hf_config", None)
    if hf_cfg is None:
        return
    cfg_type = type(hf_cfg)
    # Prefer HF API if available
    if hasattr(hf_cfg, "to_dict") and hasattr(cfg_type, "from_dict"):
        new_cfg = cfg_type.from_dict(hf_cfg.to_dict())
    elif hasattr(hf_cfg, "to_dict") and hasattr(cfg_type, "__call__"):
        # Fallback: try constructor with dict
        new_cfg = cfg_type(**hf_cfg.to_dict())
    else:
        # Last resort: shallow copy to break reference to old class object
        new_cfg = cfg_type(**getattr(hf_cfg, "__dict__", {}))
    model_config.hf_config = new_cfg
...
class ExternalZeroMQDistributedExecutor(Executor):
    ...
    def _init_executor(self) -> None:
        ...
        _reinit_hf_config_class_inplace(self.vllm_config)
